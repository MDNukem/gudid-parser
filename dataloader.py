...
class GUDIDLoader(object):
    ...
    def reqs_present(self):
        ...
        for fname in self.__class__.schema_requirements:
            if os.path.exists(
                os.path.join(
                    self.base_path,
                    self.__class__.schema_dir,
                    fname)) == False:
                missing.append(fname)

        for fname in self.__class__.data_requirements:
            if os.path.exists(os.path.join(
                    self.base_path, self.__class__.data_dir, fname)) == False:
                missing.append(fname)
    ...
    def validate_xml(self):
        assert self.reqs_fulfilled, 'Cannot validate XML, the required files are not all present'
    ...
    def create_database(self):
        ...
        '''
        if not self.reqs_fulfilled:
            self.reqs_present()

        if not self.reqs_validated:
            self.validate_xml()

        assert self.reqs_fulfilled, 'Cannot create the database, there are missing XML files'
        assert self.reqs_validated, 'Cannot create the database, the XML files could not be validated'
