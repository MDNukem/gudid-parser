...
class SqliteDatabase (BaseDatabaseWrapper):
	...
	def get_new_connection(self):
		...
		self.connection = sqlite3.connect(self._connection_params["database_location"])
		self.cursor = self.connection.cursor()

	def _verify_database_schema(self):
		raise NotImplementedError('subclasses of SqliteDatabase require a _verify_database_schema() method')
	...
	def _build_table_schema_str(self, tablename, col_list):
		...
		create_str = "CREATE TABLE {}(".format(tablename)+','.join(col_names)+")"
		return create_str
	...
	def _verify_database_schema(self, schema_file):
		...
		for table_name in table_names:
			if table_name not in existing_tables:
				...
				if self._table_has_cols(table_name, table_names[table_name]) == False:
					print("The current db schema doesn't support writing new objects of type "+str(self.__class__))
					return False
		return True
